    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 lg:hidden">
          <button
            onClick={() => onOpenQuote?.()}
            className="sm:hidden px-3.5 py-1.5 text-xs font-medium bg-[#051A24] text-white rounded-full min-h-[38px] flex items-center justify-center shadow-sm"
          >
            Quote
          </button>
          <button
            ...
            className="p-2 text-[#051A24] hover:bg-slate-100 rounded-xl transition-colors cursor-pointer min-w-[44px] min-h-[44px] flex items-center justify-center"
          >
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-5 sm:px-6 py-5 flex flex-col gap-4 animate-fade-in-up shadow-xl">
          ...
          <div className="flex flex-col gap-1">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                className="text-base font-medium text-[#051A24] hover:text-emerald-700 py-2.5 px-1 flex items-center justify-between transition-colors min-h-[44px]"
              >
          ...
          <div className="pt-3 border-t border-slate-100 flex flex-col gap-2.5">
            <Button
              ...
              className="w-full justify-center !py-3 text-sm min-h-[44px]"
            >
              Request a quote
            </Button>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center text-xs font-mono font-medium text-[#051A24] py-3 rounded-full border border-slate-200 hover:bg-slate-50 transition-colors min-h-[44px]"
            >
              <span>WA Desk: {COMPANY_INFO.whatsappNumber}</span>
            </a>
          </div>
        </div>
      )}
